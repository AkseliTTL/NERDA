...
class NERDA:
    ...
    def evaluate_performance(self, dataset: dict, 
        ...
        if return_auroc:
            ...
            sm = torch.nn.Softmax()
            print(sm(torch.from_numpy(np.asarray(flatten(probs_predicted), dtype=float))))
        else:
        ...
        if return_accuracy:
            ...
            if return_auroc:
                auroc = compute_roc_auc_score(y_pred = sm(torch.from_numpy(np.asarray(flatten(probs_predicted), dtype=float))),
                                            y_true = dataset.get('tags'),
                                            labels = self.tag_scheme)
